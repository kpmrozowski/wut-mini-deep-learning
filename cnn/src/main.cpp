...
int main(int argc, char **argv) {
    ...
    for (const auto& setting : settings) {    
...
#endif
        regularization_type = std::get<0>(setting);
        regularization_lambda = std::get<1>(setting);
        augumentation_type = std::get<2>(setting);
        experiment_name = std::get<3>(setting);
        experiment_type_idx = std::get<4>(setting);
        // Imagenette dataset
        ...
        for (int run_idx = 0; run_idx < RUNS_COUNT; ++run_idx) { 
            ...
            logs_path += "RUN_";
            logs_path += std::to_string(run_idx);
            logs_path += ".csv";

            util::CSVLogger g_logger(logs_path, 
