...
plot_model(sr.m, 
           to_file = './models_rnn/{}_{}.png'.format(arch,date), 
           show_shapes = True)

#callbacks, remember to make folders to store files 
checkpointer = ModelCheckpoint(filepath='./models_rnn/{}_{}_best.h5'.format(arch, date),
                               verbose=0,
                               save_best_only=True)
   
earlystopping = EarlyStopping()

if not os.path.exists('/logs_rnn'):
   os.makedirs('/logs_rnn')
tensorboard = TensorBoard(log_dir = './logs_rnn/{}_{}'.format(date, time()), 
                          histogram_freq = 0, 
                          write_graph = True, 
                          write_images = True)
...
sr.m.save_weights("./models_rnn/{}_{}_last.h5".format(arch, date)) 
