...
if not os.path.exists('/models_cnn'):
   os.makedirs('/models_cnn')
...
checkpointer = ModelCheckpoint(filepath="./models_cnn/ctc_{}_best.h5".format(date),
                               verbose=0,
                               save_best_only=True)
tensorboard = TensorBoard(log_dir = './logs_cnn/ctc_{}_{}'.format(date, time()), 
                          histogram_freq = 0, 
                          write_graph = True, 
                          write_images = True)
...
sr_ctc.m.save_weights('./models_cnn/ctc_{}.h5'.format(date))
sr_ctc.tm.load_weights('./models_cnn/ctc_{}_best.h5'.format(date))
